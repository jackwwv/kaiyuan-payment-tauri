...
export default function room(data, method) {
  ...
  window.sendTimeOut = setTimeout(() => {
    ...
    if (method == "chat") {
      ...
      modifyMsgStatus(data.data || data, store.getters.FAIL);
    }
  ...
  var res = new Promise((resolve, reject)=>{
    let encryptStr = rsaEncode(data, import.meta.env.VUE_APP_PUBLIC_KEY);
    window.apiSocket.emit(method, encryptStr, (recv) => {
      console.log("发送消息后emit回复" + JSON.stringify(recv));
      if(!recv){
      ...
      response(recv)
        .then((res) => {
          if (res.data.action === "chat") {
            clearTimeout(window.sendTimeOut);
            modifyMsgStatus(data.data || data, store.getters.SUCCESS);
          }
