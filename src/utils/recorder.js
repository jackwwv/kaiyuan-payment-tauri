...
var rec
/** 调用open打开录音请求好录音权限**/
export var recOpen = function(success, fail) {
...
export function recStop(callback) {
  ...
  try {
    ...
    console.error('录音停止时发生错误:', error);
    if (rec) {
      try {
        rec.close();
      } catch (closeError) {
        console.error('关闭录音器时发生错误:', closeError);
      }
      rec = null;
    }
    if (callback) {
