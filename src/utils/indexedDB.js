...
export function getLocalRoomMsg(room_uuid, page, per_page) {
    ...
    return db.transaction('rw', db.roomMsg, async() => {
        ...
        if(offset < -per_page){
            ...
            data['list'] = await db.roomMsg.where({'room_uuid':room_uuid}).offset(offset).limit(per_page).sortBy('created_at')
        }
        // 不需要再次reverse，保持created_at升序
        data['total'] = count
        return data   
    }).catch(e => {
        console.log(e.stack || e);
 ...
 *return bool
 */
 export function updateMsg(value) {
    //申明数据库
    const db = new Dexie("msg");
    //定义字段
    db.version(1).stores({ roomMsg: "++id, room_uuid, msg_id, content, created_at, updated_at" });
    //事务读写
    return db.transaction("rw", db.roomMsg, async () => {
        //更新消息
        let updated = await db.roomMsg.where({"msg_id": value.msg_id}).modify(value);
        if (updated) {
            return true   
        }
        return Promise.reject("update error") 
