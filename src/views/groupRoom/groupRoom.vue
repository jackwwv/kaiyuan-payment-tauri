 ...
 * @LastEditors: hua
 * @LastEditTime: 2020-11-22 18:53:32
 -->
<template>
  <div class="chat-room-container">
    ...
    <div class="chat-header">
      <div class="header-left">
               <role-avatar 
                 :imgUrl="serviceAvatar" 
                 role="service" 
                 size="large" 
                 :forceDefault="true"
                 class="service-avatar" 
               />
        <span class="service-name">开云客服</span>
      </div>
    ...
    <div class="chat-main">
          ...
          <div class="mscroll-container">
            ...
            <ul>
              <li v-for="(key, index) in msgList" :key="index">
                ...
                <!-- 用户消息 -->
                <div class="chat-item user-message" v-if="(key.user_id == userInfo.id)&& (key.user_type!=1) ">
                  <div class="message-bubble user-bubble">
                    ...
                    <div v-on:click="handleDefsForSelf(key)" v-else-if="key.type == TEXT" class="rawMsg">{{formatFace(key.msg)}}</div>
                    <div v-else-if="key.type == IMG" class="rawMsg" v-html="key.msg"></div>
                ...
                <!-- 客服消息 -->
                <div class="chat-item service-message" v-else>
                  <role-avatar 
                    :imgUrl="key.head_img" 
                    role="service" 
                    size="medium" 
                    :forceDefault="true"
                    class="service-avatar-small" 
                  />
                  <div class="message-bubble service-bubble">
                    <div v-if="key.type == RECORD" class="msg" @touchstart="amrPlay(JSON.parse(key.msg), index)">
                      <img class="chat_right vioce_start" :src="'static/img/voice_right.gif'" v-show="JSON.parse(key.msg)['status']" />
                      <i class="vioce_stop_right" v-show="!JSON.parse(key.msg)['status']"></i>
                      <span class="vioce_second">{{JSON.parse(key.msg)['duration']}}s</span>
                    </div>
                    <div v-else-if="key.type == TEXT" class="rawMsg" v-on:click="handleDefsForOther(key)">{{formatFace(key.msg)}}</div>
                    <div v-else-if="key.type == IMG" class="rawMsg" v-html="key.msg"></div>
                    <div v-else-if="key.type == FILE" class="rawMsg" @click="handleDefMsg(key.msg)">[文件]{{formatFileName(key.msg)}}</div>
                ...
                <!-- 系统消息 -->
                <div class="system-message" v-if="key.type == CHAT_NOTIFY && key.user_type == 1">
                  <div class="system-bubble">{{key.msg}}</div>
                </div>
              </li>
            </ul>
            <!-- 暂无消息 -->
      ...
      <div class="common-questions">
        ...
        <div class="questions-content">
          <!-- 常见问题列表 -->
          <div class="question-item" v-for="(question, index) in commonQuestions" :key="index" @click="selectQuestion(question)">
            {{question.title}}
          </div>
        </div>
      </div>
    ...
    <div class="chat-input-area">
      <div class="input-wrapper">
        ...
        <div class="input-actions">
          <button class="action-btn emoji-btn" @click="handleIconsShow">
            <yd-icon name="uniE905" custom></yd-icon>
          </button>
          <button class="action-btn image-btn" @click="handleImageUpload">
            <yd-icon name="image" custom></yd-icon>
          </button>
          <button class="action-btn file-btn" @click="handleFileUpload">
            <yd-icon name="folder" custom></yd-icon>
          </button>
          <button class="send-btn" @click="sendMsg" :disabled="!content.trim()">
            发送
            <yd-icon name="arrow-right" custom></yd-icon>
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      adderssBookList: [],
      reqImgData: {
        url: import.meta.env.VUE_APP_CLIENT_SOCKET,
        imgDatas: ""
      },
      reqData: {
        page_no: 1,
      ...
      // 新增数据
      serviceAvatar: '/static/img/service-avatar.png', // 客服头像
      commonQuestions: [
        { title: '如何注册账号？', content: '请按照以下步骤注册...' },
        { title: '如何充值？', content: '您可以通过以下方式充值...' },
        { title: '如何提现？', content: '提现需要满足以下条件...' },
        { title: '忘记密码怎么办？', content: '可以通过手机验证码重置密码...' },
        { title: '如何联系客服？', content: '您可以通过以下方式联系我们...' }
      ]
    };
  },
  created() {
  ...
  mounted() {
    ...
    title[0].innerHTML = this.$route.meta.title
  },
  ...
  methods: {
    ...
    init() {
      console.log(this.msgList,9999999);
      console.log("路由参数", this.$route.query);
      
      // 处理路由参数
      if (this.$route.query.name) {
        // 更新页面标题
        this.$store.dispatch("updateNavbarTitle", this.$route.query.name);
      }
      ...
      if (!!file) {
        ...
        reader.onload = e => {
          ...
          uploadFile({
            ...
            .then(res => {
              let file_path = import.meta.env.VUE_APP_CLIENT_SOCKET + res.data.path;
    ...
    insertIcon(src) {
      this.iconContent=src
      const arr=src.split('')
      this.iconContent=this.formatFace(arr[arr.length-1])
      console.log(src.split(''),159753);
      // this.content = `<span>${src}</span>`;
      this.content = `${this.content}${this.iconContent}`;
    },
    handleStartRecord() {
      this.defsShow = false;
      this.iconsShow = false;
      ...
      recStop(blob => {
        ...
        reader.onload = e => {
          uploadFile({
            ...
            // 动态导入 benz-amr-recorder
            const { default: BenzAMRRecorder } = await import("benz-amr-recorder");
            var amr = new BenzAMRRecorder();
            const url = import.meta.env.VUE_APP_CLIENT_SOCKET + res.data.path;
            amr
              .initWithUrl(url)
              .then(() => {
                chatSend({
          ...
          function(file) {
            ...
            reader.onloadend = function(e) {
              uploadFile({
                ...
                const { default: BenzAMRRecorder } = await import("benz-amr-recorder");
                var amr = new BenzAMRRecorder();
                let url = import.meta.env.VUE_APP_CLIENT_SOCKET + res.data.path;
                amr
                  .initWithUrl(url)
                  .then(function() {
                    chatSend({
