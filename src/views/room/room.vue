    ...
    <mescroll-vue :up="mescrollUp" :down="mescrollDown" @init="mescrollInit" @touchstart="closeDefIconsShow()">
      <div class="mscroll-container">
        <!-- <div v-show="moreInfoShow" class="more_info" @click="$router.push({ name: 'roomMsgList' })"> -->
          <div class="more_info" @click="$router.push({ name: 'roomMsgList' })">
          更多消息，请
          ...
          <li v-for="(key, index) in msgList" :key="index">
            ...
            <div class="chat-item" v-if="(key.user_id == userInfo.id) && (key.user_type != 1)">
              <div class="mychat">
                ...
                <div v-on:click="handleDefsForSelf(key)" v-else-if="key.type == TEXT" class="rawMsg" v-html="key.msg">
                  {{key.msg}}</div>
                <div  @touchstart="ocrStart(key,$event)" @touchend="ocrEnd" v-else-if="key.type == IMG" class="rawMsg" v-html="key.msg">{{key.msg}}</div>
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      const mescrollElements = document.getElementsByClassName("mescroll");
      this.mescrollDom = mescrollElements && mescrollElements.length > 0 ? mescrollElements[0] : null;
    ...
    ocrStart(item, event) {
      ...
      this.menuTop = event.touches[0].clientY;
    ...
    handleDefsShow() {
      ...
      const editDivElements = document.getElementsByClassName("edit-div");
      if (editDivElements && editDivElements.length > 0) {
        editDivElements[0].blur();
      }
      if (this.defsShow) {
        this.isPartChatPage = false;
        this.handleHeightToBottom();
    ...
    async amrPlay(rawData, index) {
      ...
      // 动态导入 benz-amr-recorder
      var { default: BenzAMRRecorder } = await import("benz-amr-recorder");
      var amr = new BenzAMRRecorder();
