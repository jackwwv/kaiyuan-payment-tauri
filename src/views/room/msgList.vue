        ...
        <mescroll-vue  ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
            <div style="display:flex;flex-direction:column">
                <div v-for="(item, index) in list" :key="index">
                    ...
                    <div class="msg" v-if="item.type == IMG" v-html="item.msg"></div>
                    <div class="msg" v-else-if="item.type == TEXT" v-html="item.msg"></div>
                    <div class="msg" v-else>{{formatMsg(item)}}</div>
                </div>
    ...
    formatMsg(data){
      ...
      try{
        if(data['type'] == this.IMG ){
          return '[图片]';  // 改回显示文字，因为图片HTML将由模板直接渲染
        }
