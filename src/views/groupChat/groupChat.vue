...
<template>
    <div class="content">
        <!-- 头部开始 -->
		<header  class="yd-navbar" :style="{width:windowSize.width+'px'} ">
        ...
        <div class="address-book-list">
            ...
            <dl>
                <!-- <dt ref="#">#</dt> -->
                <yd-checkbox-group v-model="checkedUsers"  color="#45BAF4">
                    <dd  v-for="(item, index) in adderssBookList" :key="index" v-if="item.users.first_word == '#'"> 
                        <a>
                            <vImg :imgUrl="item.users.head_img"/>
                            <yd-checkbox  :val="JSON.stringify(item.users)">{{item.users.nick_name}}</yd-checkbox>
                        </a>
...
export default {
    ...
    methods: {
        init(){
            ...
            addressBookGet(this.reqData).then(res=>{
                ...
                let adderssBookList = res.data.addressBookList.filter(item=>{return item.type == 0});
                // console.log(" this.adderssBookList", this.adderssBookList)
                let personList = []
                let unInRoom = []
                this.personList.forEach(item=>{
                    personList.push(item.users.email)
                })
                adderssBookList.forEach(item=>{
                    if(!personList.includes(item.users.email)){
                        unInRoom.push(item)
                    }
                })
                this.adderssBookList = unInRoom
            })
        },
    ...
    created() {
        ...
        console.log(width>=750,5555555555);
        if(width>=750){
          this.width=750
        }else{
          this.width=width
